...
public class ProcessAction
{
	...
	public WebResponse processRequest(WebReflectRequest reflectRequest)
	{
		// we call the action.
		if(reflectRequest.getObjectID() == null)
		{
			return process(reflectRequest.getAction(), reflectRequest.getClassParam(), reflectRequest.getMethod(), reflectRequest.getMethodParam(), reflectRequest.getMethodParamClass());
		}
		else
		{
			return process(reflectRequest.getObjectID(), reflectRequest.getAction(), reflectRequest.getMethod(), reflectRequest.getMethodParam(), reflectRequest.getMethodParamClass());
		}		
	} 
	 ...
	 * @return WebResponse
	 */
	public WebResponse process(String strClass, Object[] strClassParam, String strMethod, Object[] aParameter, Class[] aParameterClass)
	{
		WebResponse webResponse = null;
		try
		{
			// we call the process action method
			return processAction(strClass, strClassParam, strMethod, aParameter, aParameterClass);
		}
	 ...
	 * @throws Exception
	 */
	public WebResponse process(String objectID, String actionName, String strMethod, Object[] aParameter, Class[] aParameterClass)
	{
		WebResponse webResponse = null;
		try
		{
			...
			IWebObjectStorage storage = getWebObjectStorage(actionName);
			// We get the object from the storage
			Object object = storage.getObject(objectID);
			
			// we call the process action method
			return processMethod(object, strMethod, aParameter, true, aParameterClass);
		}
		catch(ConnectionFailed ce)
		{
			// for the moment we just use a standard Exception : TODO create TANGOWEB Exceptions
	 ...
	 * @throws Exception
	 */
	private WebResponse processAction(String className, Object[] aClassParam, String methodName, Object[] aParameter, Class[] aParameterClass) throws Exception
	{
	    try {
	    	System.out.println("processAction : " + className);
	    	// we create an instance of the action class
	    	Object object = createClass(className,  aClassParam);

	        // we execute the method
	        return processMethod(object, methodName, aParameter, false, aParameterClass);
	 ...
	 * @throws Exception
	 */
	private Object createClass(String className, Object[] aClassParam) throws Exception
	{
	    try {

			System.out.println("WebApplicationConfig.getSingleton().getRules().getDestClass(className); in : '" + className+"'");
			String tempStrClass = WebApplicationConfig.getSingleton().getRules().getDestClass(className);
	 ...
	 * @throws Exception
	 */
	private WebResponse processMethod(Object object, String methodName, Object[] aParameter, boolean bIsSaved, Class[] aParameterClass) throws Exception
	{
	    try {
	    	
	    	// we retrieve the parameters
	    	aParameter = retrieveObject(aParameter);
	        	...
	        	if(ret.getClass().isArray())
	        	{
	        		if(!ret.getClass().getComponentType().isPrimitive())
	        		{
		        		...
		        		if(aRet.length > 0)
				        	...
				        	{
				        		storage = getWebObjectStorage(ret.getClass().getComponentType().getName());
	        		...
	        		if(!ret.getClass().isPrimitive())
	        		{
			        	if(!(ret instanceof java.io.Serializable) || (ret instanceof AttributeValue ))
			        	{
			        		...
			        		storage = getWebObjectStorage(className);
			        		ret = SerializeObject.serializeObject(storage, ret); //serializeResult(m_webStorage,ret);
			        	}
	        		}
	        }
	        if(!bIsSaved)
	        {
		        ...
		        IWebObjectStorage storage = getWebObjectStorage(className);
	 ...
	 * @return Object[]
	 */
	public Object[] retrieveObject(Object[] aParameter) throws Exception
	{
		...
		for(int i=0;i<aParameter.length;i++)
		{
			if( (aParameter[i] instanceof WebSerializerObject) && (aParameter[i] != null))
			{
				...
				tempStorage = getWebObjectStorage(className);
				...
				else
				{
					...
					aNewParam[i] = createClass(className,  aClassParam);
					if(objectID != null)
	 ...
	 */
	public IWebObjectStorage getWebObjectStorage(String className)
	{
