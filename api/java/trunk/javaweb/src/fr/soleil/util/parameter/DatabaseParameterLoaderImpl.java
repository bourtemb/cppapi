...
public class DatabaseParameterLoaderImpl implements IParameterLoader
{
	...
	public DatabaseParameterLoaderImpl(String defaultENVKey)
	{
			...
			boolean bReset)
	{
		...
		try
		{
			...
			String strQuery = "SELECT IUID, SCATEGORY, SKEY, SVALUE from USERPARAMETERS where IUID = " + m_strApplicationIUID + " AND SCATEGORY = '" + m_strENVKey+"_" + strParameterName+strEndfileName + "'"; 
			...
			// If reset data is required we simply replace the old properties by the new properties file else we add the new properties 
        	if(bReset)
	...
	/**
	 * Return the env key : REC, INT, PROD, or the name of the workstation for DEVs.
	 * @return String
	 */
	...
	/**
	 * Change the env key : REC, INT, PROD, or the name of the workstation for DEVs.
	 * @param key
	 */
