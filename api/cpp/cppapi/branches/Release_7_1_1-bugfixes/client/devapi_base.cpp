...
//
	...
	unsigned int i;
	if (attr_string_list.size() > 1)
	{
		vector<string> same_att = attr_string_list;
		...
		int duplicate_att = distance(attr_string_list.begin(),attr_string_list.end()) - distance(same_att.begin(),pos);
		if (duplicate_att != 0)
		{
			TangoSys_OMemStream desc;
			...
			desc << ends;
			ApiConnExcept::throw_exception((const char*)"API_AttributeFailed",
