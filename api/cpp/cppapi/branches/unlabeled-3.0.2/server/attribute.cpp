...
//
	...
	ext->rel_change[0] = INT_MAX;			// default for relative change is 10% 
	ext->rel_change[1] = INT_MAX;			// default for relative change is 10% 
	...
	ext->event_period = 1000;  	// default for event period is 1 second
	...
	try
	{
			...
			long event_period=0;
                	...
                	str >> event_period;
			if (event_period > 0) ext->event_period = event_period;
			cout1 << "Attribute::Attribute(): event_period_str " << event_period_str << " event_period = " << ext->event_period << endl;
        	}                                 
	}
	catch (...) { }

	ext->archive_period = 10*ext->poll_period;  	// default for event period is 10*poll_period
	if (ext->archive_period <= 0) ext->archive_period = 1;
	...
	try
	{
        	...
        	{
			long archive_period;
			str.seekp(0); str.seekg(0);
                	str << archive_period_str << ends;
			str >> archive_period;
			if (archive_period > 0) ext->archive_period = archive_period;
			cout1 << "Attribute::Attribute(): archive event_period = " << ext->event_period << endl;
        	}                                 
	}
	catch (...) { }
	...
	ext->prev_periodic_event.inited = false;
//
