...
//
    		...
    		Logging::init(ds_name, orb, (int)_tracelevel, _UseDb);
...
//
		...
		try
		{
			event_supplier = EventSupplier::create(orb, ds_name, db);
			event_supplier->connect();
		}
		catch (...)
		{
			cout1 << "could not create EventSupplier, try again later ...\n";
			event_supplier = NULL;
		}
