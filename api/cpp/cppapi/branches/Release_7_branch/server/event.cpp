...
///
///		$Log$
///		Revision 1.22.2.1  2007/12/19 15:54:47  taurel
...
void EventConsumer::attr_to_device(const AttributeValue *attr_value,
				   ...
				   const AttributeValue_4 *attr_value_4,
				   long vers,DeviceAttribute *dev_attr)
{
	...
	DevVarEncodedArray *tmp_seq_enc;
	...
	if (dev_attr->quality != Tango::ATTR_INVALID)
	{
		if (vers >= 4)
		{
			if (attr_value_4->enc_value.length() != 0)
			{
				max = attr_value_4->enc_value.maximum();
				len = attr_value_4->enc_value.length();
				if (attr_value_4->enc_value.release() == true)
				{
					tmp_seq_enc = const_cast<Tango::DevVarEncodedArray *>(&(attr_value_4->enc_value));
					tmp_enc = tmp_seq_enc->get_buffer((CORBA::Boolean)true);
					dev_attr->ext->EncodedSeq = new DevVarEncodedArray(max,len,tmp_enc,true);
				}
				else
				{
					tmp_enc = const_cast<Tango::DevEncoded *>(attr_value_4->enc_value.get_buffer());
...
//
					...
					if (ty->kind() == tk_struct)
					{
						...
						if (::strcmp(tmp_ptr,"AttributeValue_4") == 0)
						{
							...
							vers = 4;
							attr_to_device(attr_value,attr_value_3,attr_value_4,vers,dev_attr);
						}
...
//
 
 		{
			...
			//cout << "keep alive taking reader lock!" << endl;
			...
			for (ipos = event_consumer->channel_map.begin(); ipos != event_consumer->channel_map.end(); ipos++)
			{
				try
				{
					...
					ipos->second.channel_monitor->get_monitor();
					//cout << "keep alive taking channel monitor for " << ipos->first << endl;
						...
						for (epos = event_consumer->event_callback_map.begin(); epos != event_consumer->event_callback_map.end(); epos++)
						{
							if (epos->second.channel_name == ipos->first )
							{
								try
								{
									// lock the callback 
									epos->second.callback_monitor->get_monitor();
									//cout << "keep alive taking callback monitor : resubscribe : " << epos->first<< endl;
									...
									epos->second.callback_monitor->rel_monitor();
									//cout << "keep alive release callback monitor : resubscribe" << endl;
								}
								catch (...) 
								{
									epos->second.callback_monitor->rel_monitor();
									cerr << "EventConsumerKeepAliveThread::run_undetached() exception during periodic resubscription of " << epos->first << endl;
								};	
							}
						}
						//event_consumer->cb_map_mut.unlock();
		...
		//
						...
						for (epos = event_consumer->event_callback_map.begin(); epos != event_consumer->event_callback_map.end(); epos++)
						{
							if ((epos->second.channel_name == ipos->first) && (epos->second.callback != NULL))
							{
								...
								{
									epos->second.callback_monitor->get_monitor();
									//cout << "keep alive get callback monitor : event : " << epos->first << endl;
			...
			//
									...
									epos->second.callback_monitor->rel_monitor();
									//cout << "keep alive release callback monitor : event" << endl;
								}
								catch (...)
								{
					...
					ipos->second.channel_monitor->rel_monitor();
					//cout << "keep alive release channel monitor " << endl;	
				}
				catch (...)
				{
					cerr << "EventConsumerKeepAliveThread::run_undetached() timeout on callback monitor of " << epos->first << endl;
