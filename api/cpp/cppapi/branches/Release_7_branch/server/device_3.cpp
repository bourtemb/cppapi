			...
			if (qual != Tango::ATTR_INVALID) 
			{
				...
				else
				{
					try
					{
						...
						CORBA::Any *a_ptr;
						if (back != NULL)
							a_ptr = &((*back)[index].value);
						else
							a_ptr = &((*back4)[index].value);
						...
						case Tango::DEV_ENCODED :
						{
							...
							{
								Tango::DevEncoded *ptr = att.get_encoded_value();
								if (w_type == Tango::READ)
									(*back4)[index].enc_value.length(1);
								else
									(*back4)[index].enc_value.length(2);
								(*back4)[index].enc_value[0].encoded_format = CORBA::string_dup(ptr->encoded_format);
								(*back4)[index].enc_value[0].encoded_data.replace(ptr->encoded_data.length(),ptr->encoded_data.length(),ptr->encoded_data.get_buffer());
							}
		...
		{
			{
				...
				CORBA::Any *polled_attr_any;
				Tango::AttrQuality qual;
				bool data_in_any = true;
				
				if (back != NULL)
				{
					AttributeValue_3 &att_val = polled_attr->get_last_attr_value_3(false);
					polled_attr_any = &att_val.value;
				...
				else
				{
					...
					polled_attr_any = &att_val.value;
					qual = att_val.quality;
					unsigned int nb_encoded = att_val.enc_value.length();
					if (nb_encoded != 0)
					{
						(*back4)[i].enc_value.length(nb_encoded);
						for (unsigned int loop = 0;loop < nb_encoded;loop++)
						{
							(*back4)[i].enc_value[loop].encoded_format = CORBA::string_dup(att_val.enc_value[loop].encoded_format);
							unsigned char *tmp_enc = att_val.enc_value[loop].encoded_data.get_buffer();
							unsigned int nb_data = att_val.enc_value[loop].encoded_data.length();
							(*back4)[i].enc_value[loop].encoded_data.replace(nb_data,nb_data,tmp_enc);
						}
						data_in_any = false;
					}
				}
				
				if ((data_in_any == true) && (qual != Tango::ATTR_INVALID))
				{					
					...
					CORBA::Any *any_ptr;
					if (back != NULL)
						any_ptr = &((*back)[i].value);
					else
						any_ptr = &((*back4)[i].value);
																														
					switch (type)
					{
					case Tango::DEV_SHORT :
						(*polled_attr_any) >>= tmp_sh;
