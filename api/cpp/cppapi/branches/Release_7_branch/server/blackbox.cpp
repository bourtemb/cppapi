...
void BlackBox::insert_attr(const Tango::AttributeValueList &att_list, const ClntIdent &cl_id,long vers)
{
	sync.lock();
...
void BlackBox::insert_wr_attr(const Tango::AttributeValueList &att_list, const ClntIdent &cl_id,long vers)
{
	sync.lock();
...
void BlackBox::insert_attr_wr_nl(const Tango::AttributeValueList &att_list, long vers)
{	
//
// Insert elt in the box
//
...
//
// Add client identificatio if available
//
...
{
cout << "In client_addr copy ctor" << endl;
...
{
cout << "Client_addr assignement operator" << endl;
	client_ident = rhs.client_ident;
	client_lang = rhs.client_lang;
	client_pid = rhs.client_pid;
	java_main_class = rhs.java_main_class;
...
{
cout << "client_addr == operator" << endl;
...
ostream &operator<<(ostream &o_str,const client_addr &ca)
{
	...
	else
	{
		...
		{
			const char *tmp = ca.client_ip;
			o_str << "CPP or Python client with PID " << ca.client_pid << " from host " << tmp;
		}
		else
		{
			const char *tmp = ca.client_ip;
			o_str << "JAVA client with UUID " << ca.java_ident[0] << ", " << ca.java_ident[1] << " from host " << tmp;
		}
	}
