...
void DeviceImpl::init_attr_poll_ext_trig(string attr_name)
{
	...
	Attribute &req_attribute = dev_attr->get_attr_by_name (attr_lowercase.c_str());
...
//
	...
	if (ext->device_locked == true)
	{
		if (valid_lock() == true)
		{
			...
			dvlsa->svalue[0] = CORBA::string_dup(tmp);
		...
		else
		{
			...
			dvlsa->svalue[0] = CORBA::string_dup("Not defined");
	...
	dvlsa->svalue[2] = CORBA::string_dup(ext->lock_status.c_str());
...
//+-------------------------------------------------------------------------
//
// method : 		Device_Impl::throw_locked_exception()
// 
