...
//
	...
	static void attr_to_device(const AttributeValue *,const AttributeValue_3 *,const AttributeValue_4 *,long,DeviceAttribute *);
...
protected :
	...
	void ask_locking_status(vector<string> &,vector<long> &);
...
public :
	...
	DeviceProxy():Connection((CORBA::ORB *)NULL),adm_device(NULL),ext_proxy(NULL)
	{dbase_used = false;}
...
//
	...
	DeviceAttribute write_read_attribute(DeviceAttribute &);
...
// Locking methods
//
	void lock(int lock_validity=DEFAULT_LOCK_VALIDITY);
	void unlock(bool force=false);
	string locking_status();
	bool is_locked();
	bool is_locked_by_me();
	bool get_locker(LockerInfo &);
};
...
//
	...
	DevState state();	
	int ping();
	void set_transparency_reconnection(bool);
	bool get_transparency_reconnection();
