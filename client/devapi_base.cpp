...
//						desc << db_port << ends;
desc << db_port << "\n";
desc << "start time = " << start.tv_sec << "," << start.tv_usec << "\n";
desc << "stop time = " << stop.tv_sec << "," << stop.tv_usec << "\n" << ends;
			...
			else
			{
				...
				ApiCommExcept::re_throw_exception(one,
							      (const char*)"API_CommunicationFailed",
                        				      desc.str(),
							      (const char*)"DeviceProxy::command_inout()");
			}
		}
...
vector<DeviceAttribute> *DeviceProxy::read_attributes(vector<string>& attr_string_list) 
{
	...
	while (ctr < 2)
	{
		try 
		{
			...
			if (version >= 4)
				attr_value_list_4 = ext->device_4->read_attributes_4(attr_list,source);
			else if (version == 3)
	...
	for (i=0; i < nb_received; i++)
	{
		...
		{
			if (version == 3)
				ApiUtil::attr_to_device(NULL,&(attr_value_list_3[i]),NULL,version,&(*dev_attr)[i]);
			else
				ApiUtil::attr_to_device(NULL,NULL,&(attr_value_list_4[i]),version,&(*dev_attr)[i]);
...
DeviceAttribute DeviceProxy::read_attribute(string& attr_string) 
{
	...
	{
		if (version == 3)
			ApiUtil::attr_to_device(NULL,&(attr_value_list_3[0]),NULL,version,&dev_attr);
		else
			ApiUtil::attr_to_device(NULL,NULL,&(attr_value_list_4[0]),version,&dev_attr);
...
//
			...
			if (version >= 4)
			{
				...
				ext->device_4->write_attributes_4(attr_value_list,ci);
			}
			else if (version == 3)
		...
		else
		{
			...
			else
			{
				...
				if (pos->second.shared->cmd_pending == true)
				{
					...
					if ((pos->second.shared->cmd_pending == true) && (interupted == false))
					{
						cout4 << "TIME OUT" << endl;
...
bool DeviceProxy::is_locked_by_me()
{
	...
	else
	{
		...
		{
			string full_ip_str;
			get_locker_host(v_str[0],full_ip_str);
...
//
		...
		if (v_l[1] != 0)
		{
			...
			string full_ip;
			get_locker_host(v_str[0],full_ip);
...
//
			...
			if (ho != NULL)
				lock_info.locker_host_class = ho->h_name;
			else
				lock_info.locker_host_class = full_ip;
		}
		else
		{
			...
			lock_info.locker_host_class = v_str[1];
		}
	}
...
void DeviceProxy::ask_locking_status(vector<string> &v_str,vector<long> &v_l)
{
...
//
	...
	ApiUtil::attr_to_device(NULL,NULL,&(attr_value_list_4[0]),version,&ret_dev_attr);
