...
//
// Also initialise device_2 if it's support IDL release 2
//
...
vector<DeviceAttribute> *DeviceProxy::read_attributes(vector<string>& attr_string_list) 
{
	...
	for (unsigned int i=0; i<attr_value_list->length(); i++)
	{
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::add_logging_target - Add a logging target to the device
//
//-----------------------------------------------------------------------------
void DeviceProxy::add_logging_target(const string &target_type, 
                                     const string &target_name)
{
	...
	in.length(2);
  	...
  	std::string tn = target_type + "::" + target_name;

	in[0] = CORBA::string_dup(device_name.c_str());
	in[1] = CORBA::string_dup(tn.c_str());
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::remove_logging_target - Add a logging target to the device
//
//-----------------------------------------------------------------------------
void DeviceProxy::remove_logging_target (const string &target_type, 
                                         const string &target_name)
{
	if (adm_device == NULL)
		connect_to_adm_device();
