...
//
	...
	env_var_fqdn_prefix = "tango://" + db->get_db_host() + ':' + db->get_db_port() + '/' ;
...
//
	...
	{
		device_name.insert(0,env_var_fqdn_prefix);
	}
	...
	{
		channel_name.insert(0,env_var_fqdn_prefix);
	}
...
//
...

	{
		...
		try
		{
			...
			if (device_proxy->get_from_env_var() == true)
			{
				adm_name.insert(0,env_var_fqdn_prefix);
			}
		}
...
void EventConsumer::push_structured_event(const CosNotification::StructuredEvent& event)
{
	...
	if (event_name == "heartbeat")
	{
		...
		string::size_type pos = domain_name.find('/',8);
		if (event_type.find("tango://") != string::npos)
			fq_dev_name.insert(0,event_type);
		else
			fq_dev_name.insert(0,env_var_fqdn_prefix);
...
//
		...
		if (dd.is_empty() == true)
		{
			if (device->get_from_env_var() == false)
			{
				...
				if (fqdn_prefix != env_var_fqdn_prefix)
				{		
					TangoSys_OMemStream o;
					o << "Device server for device " << device_name;
					o << " is too old to generate event in a multi TANGO_HOST environment. Please, re-link with Tango >= 7.1" << ends;					
