...
//
		...
		int th_id = th->id();
		if (th_id != tg->get_polling_thread_id())
		{
	...
	if ((with_db_upd == true) && (Tango::Util::_UseDb == true))
	{
		...
		if (type == Tango::POLL_CMD)
		{
			...
			{	
				...
				{
					if (::strcasecmp(cmd_list[i].c_str(),obj_name.c_str()) == 0)
					{
		...
		else
		{
			...
			{	
				...
				{
					if (::strcasecmp(attr_list[i].c_str(),obj_name.c_str()) == 0)
					{
...
//
	...
	else
	{
		...
		PollThread *poll_th = tg->get_polling_thread_object();
...
// 
// description : 	command to ask the polling thread to send
...
// 
// description : 	command to ask the polling thread to stop sending
//			the event heartbeat 
//
//-----------------------------------------------------------------------------
